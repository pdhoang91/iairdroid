...
const fileKey = (file) => `blum_assets_${file}`;
const { exec } = newSemaphore();

export const handleMessage = async (data) => {
  const href =
    "https://raw.githubusercontent.com/namlt2882/wave-management-tool/master/assets/";
...
${e.stack}`
  ...
  async function R(e) {
    ...
    typeof e < "u" && Object.getPrototypeOf(e) === Object.prototype
      ...
      typeof e > "u" &&
        (e = new URL(
          "" + new URL("game_wasm_bg-BnV071fP.wasm", href).href,
          href
        ));
    const n = $();
    let buffer,
      fileName = "game_wasm_bg-BnV071fP.wasm";
    await exec(async () => {
      const data = getItemObj(fileKey(fileName));
      if (data) {
        ...
        (typeof e == "string" ||
          ...
          (typeof URL == "function" && e instanceof URL)) &&
          (e = fetch(e));
        console.log(`Load file ${fileName} from source`);
        buffer = Buffer.from(await (await e).arrayBuffer());
        const data = buffer.toString("base64");
        buffer = Buffer.from(data, "base64");
        setItem(fileKey(fileName), data, 30 * 24 * 60 * 60_000);
      }
    });
    const { instance: t, module: r } = await D(buffer, n);

    // const buffer = loadFile("assets/game_wasm_bg-BnV071fP.wasm");
    // const { instance: t, module: r } = await D(buffer, n);
    return L(t, r);
  }
  let x;
  ...
  await V();
  const { id: n, method: t, payload: r } = data;
  switch (t) {
    case "proof": {
      const o = F(r);
      return { id: n, ...o };
